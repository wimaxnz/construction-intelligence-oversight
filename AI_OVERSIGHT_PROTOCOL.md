...
5. **Respond (async)** — Review findings land in `AI_OVERSIGHT_RESPONSE.md` (pull with `-Pull` / `pull` when convenient).
6. **Incorporate** — Valid findings enter backlog or future sprint queues; active validated work continues.
...
When `AI_OVERSIGHT_RESPONSE.md` contains findings:
...
3. **Pull inbound (when convenient)** — `.\scripts\sync-ai-oversight.ps1 -Pull` fetches `AI_OVERSIGHT_RESPONSE.md` into `docs/ai-oversight/`. Incorporate findings per the table above; never block the next sprint on pull timing.
