/**
 * Payload validation — mirrors scripts/oversight-bridge/validate-payload.ts
 * Keep in sync when bridge validation rules change.
 */
...
export const FORBIDDEN_FIELD_NAMES = [
  'repo', 'repository', 'path', 'file', 'filepath', 'branch', 'command', 'shell',
  'cmd', 'exec', 'script', 'workflow', 'token', 'secret', 'key', 'url', 'endpoint',
  'ref', 'sha', 'commit', 'owner', 'org', 'organization', 'cwd', 'env', 'run',
  'bash', 'powershell', 'git',
] as const;
...
const REQUIRED_STRING_FIELDS = [
  'findingId', 'timestamp', 'severity', 'category', 'title',
  'evidence', 'whyItMatters', 'recommendedAction', 'source',
] as const;
...
export function validatePayloadSize(raw: string): ValidationError | null {
  ...
  if (bytes > MAX_PAYLOAD_BYTES) {
    return { code: 'PAYLOAD_TOO_LARGE', message: `Payload ${bytes} bytes exceeds max ${MAX_PAYLOAD_BYTES}` };
  }
  return null;
}
...
export function detectForbiddenFields(obj: Record<string, unknown>): string[] {
  ...
  for (const key of Object.keys(obj)) {
    ...
    if (FORBIDDEN_FIELD_NAMES.some((f) => lower === f || lower.includes(f))) hits.push(key);
  }
  return hits;
}
...
function validateStringField(name: string, value: unknown, maxLen: number): ValidationError | null {
  if (typeof value !== 'string') return { code: 'INVALID_TYPE', message: `${name} must be a string` };
  if (value.trim().length === 0) return { code: 'EMPTY_FIELD', message: `${name} must not be empty` };
  if (value.length > maxLen) return { code: 'FIELD_TOO_LONG', message: `${name} exceeds max length ${maxLen}` };
  return null;
}

export function validatePayload(raw: unknown):
  | { ok: true; payload: OversightFindingPayload }
  ...
  if (forbidden.length > 0) {
    errors.push({ code: 'FORBIDDEN_FIELDS', message: `Rejected fields: ${forbidden.join(', ')}` });
  }
  ...
  for (const field of REQUIRED_STRING_FIELDS) {
    if (!(field in obj)) errors.push({ code: 'MISSING_FIELD', message: `Missing required field: ${field}` });
  }
  ...
  if (findingIdErr) errors.push(findingIdErr);
    ...
    errors.push({ code: 'INVALID_FINDING_ID', message: 'findingId must be alphanumeric with hyphens' });
  }
  ...
  if (!VALID_SEVERITIES.includes(obj.severity as Severity)) {
    errors.push({ code: 'INVALID_SEVERITY', message: `severity must be one of: ${VALID_SEVERITIES.join(', ')}` });
  }

  if (!VALID_CATEGORIES.includes(obj.category as Category)) {
    errors.push({ code: 'INVALID_CATEGORY', message: `category must be one of: ${VALID_CATEGORIES.join(', ')}` });
  }
  ...
  if (obj.source !== VALID_SOURCE) {
    errors.push({ code: 'INVALID_SOURCE', message: `source must be "${VALID_SOURCE}"` });
  }
